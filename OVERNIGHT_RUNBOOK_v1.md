...
```bash
pip install duckdb lightgbm feedparser pytest pytest-asyncio redis httpx pandas numpy scipy pydantic-settings sqlalchemy loguru orjson yfinance pandas_ta tenacity 2>&1 | tail -3
python3 -m scripts.status
```

**PASS**: `scripts.status` shows green on every required dep. If a dep is red, `pip install <name>` it.
...
**PASS**: `CUDA available: True`, device shows `5080`, VRAM ~16GB. If False, `pip install torch --index-url https://download.pytorch.org/whl/cu121` then re-test.

### 1.3 CuPy (optional, used by sweeper for GPU metrics)
...
try:
    ...
    print('CuPy not installed — sweeper falls back to numpy (still works)')
"
```

PASS: either "CuPy OK" or the graceful fallback message. Not blocking.
...
Run migrations 007-016 in order. **Do not skip any.**

```bash
for n in 007 008 009 010 011 012 013 014 015 016; do
  echo "=== Migration $n ==="
...
**PASS**: each migration completes without error; the `\dt` listing shows tables `signal_ranks`, `model_runs`, `backtest_runs`, `regime_forecasts`, `whale_flow_signals`, `short_squeeze_signals`, `reddit_signals`, etc.
...
### 6.5 Confirm all 3 jobs are running
