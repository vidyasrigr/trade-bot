...
# Reports live at repo-root data/backtest_reports (alongside yesterday's runs),
# while the MarketData cache lives at backend/data/marketdata_cache. Resolve the
# repo root from this file so output location is independent of cwd.
REPO_ROOT = Path(__file__).resolve().parents[2]
REPORTS_DIR = REPO_ROOT / "data" / "backtest_reports"
...
# The 20 names whose train+wf chains are FULLY cached from yesterday's runs.
# Running the VRP sweep on these costs ~0 new credits (cache hits only), so the
# first master pass finds the best wing distance / stop for free. Phase C then
# spends credits confirming the winner on VRP_UNIVERSE_FULL.
VRP_UNIVERSE_CACHED20 = [
    "SPY", "QQQ", "IWM", "AAPL", "MSFT", "NVDA", "AMZN", "META", "GOOGL", "TSLA",
    "AMD", "JPM", "BAC", "XOM", "UNH", "LLY", "COST", "WMT", "DIS", "NFLX",
]

# Active universe for this run (override via --vrp-universe full).
VRP_UNIVERSE = VRP_UNIVERSE_CACHED20
...
def build_variant_grid() -> list[Variant]:
    ...
    # 12-1 (252d), 9-1 (189d), 6-1 (126d), 3-1 (63d) — academic momentum windows.
    for lookback in (252, 189, 126, 63):
        out.append(Variant(
    ...
    # skew_25d cross-section — validated on BS-inverted IV from cached chains (free).
    for hold in (21, 42):
        out.append(Variant(
            signal="skew_25d",
            name=f"skew_25d_hold={hold}d",
            config={"hold_days": hold, "rebalance_days": 21, "quantile": 0.3,
                     "universe": "liquid_1000"},
            needs_marketdata=False, universe_size=1000,
        ))
    ...
    # pre_fomc_straddle is an OPTIONS structure, not an equity position — it needs
    # the options-engine path, not the equity generator contract. Deferred to a
    # dedicated builder (Phase C); omitted here so verdicts stay clean.
    ...
    # Regime-gated entries (skip opening short vol into accelerating-vol days) —
    # post-hoc tail fix after IC + stops both failed in the first sweep.
    for ratio in (1.3, 1.5):
        out.append(Variant(
            signal="vrp_harvest",
            name=f"vrp_regime_gate_ratio={ratio}",
            config={"variant": "regime", "stress_ratio": ratio,
                     "iv_rank_min": 50, "vrp_z_min": 1.0,
                     "profit_target": 0.5, "stop_loss": 2.0, "short_delta": 0.16},
            needs_marketdata=True, universe_size=40, estimated_credits=0,
        ))
...
# One shared MarketData source for ALL VRP variants. Critical: separate sources
# do NOT share the in-memory cache, so concurrent variants would re-fetch the
# same uncached chains N times. A shared source + sequential VRP execution means
# the first variant warms the disk cache and every later variant reads it free.
_SHARED_VRP_SOURCE = None


def _vrp_source():
    global _SHARED_VRP_SOURCE
    if _SHARED_VRP_SOURCE is None:
        from backtest.marketdata_source import MarketDataHistoricalSource
        _SHARED_VRP_SOURCE = MarketDataHistoricalSource()
    return _SHARED_VRP_SOURCE


async def _run_vrp_variant(v: Variant) -> dict:
    """Runs both train + walk-forward for a VRP variant from the shared source."""
    ...
    elif variant_kind == "regime":
        from backtest.strategies.vrp_regime import run_vrp_regime_backtest
        runner = run_vrp_regime_backtest
        extra_kwargs = {"stress_ratio": v.config.get("stress_ratio", 1.3)}
    else:
        runner = run_naked
        extra_kwargs = {}
    ...
    source = _vrp_source()
    before = source.stats.get("api_fetches", 0)
    ...
    return {
        ...
        "credits_burned": source.stats.get("api_fetches", 0) - before,
        "universe_size": len(VRP_UNIVERSE),
    }


async def _resolve_universe(size: int) -> list[str]:
    """
    A LIQUID symbol list for equity-signal backtests.

    NOTE: the laptop draft used get_scan_universe()[:size], but that returns the
    ~5,000-name Nasdaq directory sorted ALPHABETICALLY — first-N is illiquid junk,
    not "top by ADV". The curated get_full_universe() (~150 liquid large/mid caps)
    is the honest, reliable choice. True ADV-ranked breadth is a Phase C expansion.
    """
    from data.scanner import get_full_universe
    return get_full_universe()[:size]
...
async def _run_momentum_variant(v: Variant) -> dict:
    ...
    try:
        ...
        from backtest.strategies.momentum_xs_v2 import (
            generate_momentum_trades, _close_panel,
        )
    ...
    universe = await _resolve_universe(v.universe_size)
    panel = _close_panel(universe)  # fetch once, reuse for both folds

    train_trades = await generate_momentum_trades(
        universe, TRAIN_START, TRAIN_END,
        lookback_days=v.config["lookback_days"],
        rebalance_days=v.config["rebalance_days"], panel=panel,
    )
...
async def _run_generic_equity_variant(v: Variant, strategy_module: str,
    ...
    # Strip the "universe" selector from config — universe is passed positionally,
    # so leaving it in **config collides (TypeError: multiple values for 'universe').
    cfg = {k: val for k, val in v.config.items() if k != "universe"}

    train_trades = await generator(universe, TRAIN_START, TRAIN_END, **cfg)
    wf_trades = await generator(universe, WF_START, WF_END, **cfg)
...
VARIANT_DISPATCH = {
    ...
    "skew_25d":
        lambda v: _run_generic_equity_variant(
            v, "backtest.strategies.skew_xs", "generate_skew_trades"),
    "pre_fomc_straddle":
        lambda v: _run_generic_equity_variant(
...
async def run_one(v: Variant) -> None:
    ...
    try:
        ...
        if "error" in result and result["error"]:
            err = result["error"]
            blocked_markers = ("not built", "not present", "missing:",
                               "No module named", "Restricted", "402", "legacy")
            v.status = "blocked" if any(m in err for m in blocked_markers) else "error"
            v.error = err
        else:
...
async def run_all(variants: list[Variant], concurrency: int) -> None:
    ...
    # VRP variants share one MarketData source and MUST run sequentially so the
    # first warms the disk cache and the rest read it free. Free equity signals
    # run concurrently alongside them.
    vrp = [v for v in variants if v.signal == "vrp_harvest"]
    others = [v for v in variants if v.signal != "vrp_harvest"]

    async def run_vrp_sequential() -> None:
        for v in vrp:
            await run_one(v)
            write_progress(variants)

    await asyncio.gather(run_vrp_sequential(), *[guarded(v) for v in others])
...
async def main(args) -> int:
    ...
    global VRP_UNIVERSE
    if args.vrp_universe == "full":
        VRP_UNIVERSE = VRP_UNIVERSE_FULL
    logger.info(f"VRP universe: {len(VRP_UNIVERSE)} names ({args.vrp_universe})")
...
def cli() -> None:
    ...
    parser.add_argument("--vrp-universe", type=str, default="cached20",
                        choices=("cached20", "full"),
                        help="cached20 (free, 20 names) or full (40 names, costs credits)")
